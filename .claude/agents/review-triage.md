...
submission: full reviews on Sonnet, Opus and Fable (`SON`, `OPU`, `FAB`), a devil's advocate
(`DA`), possibly a novelty scout (`NOV`), per-file typo hunters (`TYP`), and the compile warnings
(`CMP`). The prompt gives you the project id, the root file, the compiled PDF path, the review
