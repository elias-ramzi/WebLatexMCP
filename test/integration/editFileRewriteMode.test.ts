...
describe('edit_file rewrite-preservation mode', () => {
  ...
  it(
    'an explicit preserveOriginal: true on an ineligible .md file is reported in the text, ' +
      'not silently honoured as off',
    async () => {
      const { client, userDir } = await setup();
      await writeFile(path.join(userDir, 'notes.md'), `${PARAGRAPH_OLD}\n`, 'utf8');
      await client.callTool({
      ...
      // No set_rewrite_mode call: the assertion is per-call (preserveOriginal: true), so the
      // project's stored/default mode must not matter here.
      ...
      expect(res.isError ?? false).toBe(false);
      const out = structured<{ rewriteMode: string; preservedEdits: number }>(res);
      // The structured fields report what actually applied ('off') — unchanged by this fix.
      expect(out.rewriteMode).toBe('off');
      expect(out.preservedEdits).toBe(0);
      // But the assertion that could not be honoured must be named in the text, not left
      // indistinguishable from a project genuinely configured 'off'.
      expect(textOf(res)).toContain('preserveOriginal was ignored');
      ...
      const content = structured<{ content: string }>(read).content;
      expect(content).toBe(`${PARAGRAPH_NEW}\n`);
      expect(content).not.toContain('%');
    },
  );

  it('eligibility is judged on the link-resolved name: a .tex link onto a .md preserves nothing, and says so', async () => {
    // The bytes land in the target, so the target's comment syntax is what matters (the same
    // reason the .bib guard follows `linkTarget`): a `.tex` name in front of a Markdown file
    // would otherwise write `% ` lines into a document that has no such comment. An in-project
    // link never leaves the sandbox, so it is followed regardless of `followSymlinks`.
    const { client, userDir } = await setup();
    await writeFile(path.join(userDir, 'body.md'), `${PARAGRAPH_OLD}\n`, 'utf8');
    await symlink('body.md', path.join(userDir, 'notes.tex'));
    await client.callTool({ name: 'register_project', arguments: { project: 'p', path: userDir } });
    ...
    expect(out.rewriteMode).toBe('off');
    expect(out.preservedEdits).toBe(0);
    // Named after the file the bytes landed in, not the link.
    expect(textOf(res)).toContain('preserveOriginal was ignored: body.md has no %-line-comment');

    const content = await readFile(path.join(userDir, 'body.md'), 'utf8');
    expect(content).toBe(`${PARAGRAPH_NEW}\n`);
    expect(content).not.toContain('%');
  });

  it('the .bib exemption follows the link too: a .tex link onto refs.bib, with confirmBibEdit, preserves nothing', async () => {
    const { client, userDir } = await setup();
    const OLD_ENTRY =
      '@misc{a, title={The quick brown fox jumps over the lazy dog while the sun sets}}';
    const NEW_ENTRY =
      '@misc{a, title={A swift fox leaps across the sleeping dog as evening light fades}}';
    await writeFile(path.join(userDir, 'refs.bib'), `${OLD_ENTRY}\n`, 'utf8');
    await symlink('refs.bib', path.join(userDir, 'notes.tex'));
    await client.callTool({ name: 'register_project', arguments: { project: 'p', path: userDir } });

    // Without confirmBibEdit the link is refused by the .bib guard, naming the target.
    const refused = await client.callTool({
      ...
      arguments: {
        ...
        path: 'notes.tex',
        preserveOriginal: true,
        edits: [{ oldString: OLD_ENTRY, newString: NEW_ENTRY }],
      },
    });
    expect(refused.isError).toBe(true);
    expect(textOf(refused)).toContain('is a link to \\"refs.bib\\"');
    ...
    expect(out.rewriteMode).toBe('off');
    expect(out.preservedEdits).toBe(0);
    expect(textOf(res)).toContain('preserveOriginal was ignored: refs.bib is a .bib file');

    // The bibliography got the edit and nothing else — a `%` line is not a comment in BibTeX.
    expect(await readFile(path.join(userDir, 'refs.bib'), 'utf8')).toBe(`${NEW_ENTRY}\n`);
  });
  ...
  it('a .bib file: refused without confirmBibEdit, and preserves nothing even with confirmBibEdit + always', async () => {
    ...
    // The exemption is byte-pinned, not just field-pinned: the edit below replaces the WHOLE
    // entry line (oldString starts at line start and ends with its own trailing newline), under
    // mode 'always' (both stored on the project AND asserted per-call via preserveOriginal:
    // true) plus confirmBibEdit: true. A line-aligned, non-replaceAll, mode-'always' edit is
    // exactly the shape createPreserveTransform preserves everywhere else in the suite (see "a
    // whole-line deletion under always is still preserved" above) — so if the `!isBib` narrowing
    // in editFile.ts AND the fact that '.bib' is absent from LINE_COMMENT_EXTENSIONS were both
    // removed, this same edit WOULD produce a `% @misc{a, title={Old Title Words Here}}` comment
    // line above the replacement. Asserting the exact final bytes below (no `%` anywhere, the old
    // entry text gone) therefore fails if either gate is removed — a mid-line match could not
    // make that claim, since it is never preserved regardless of mode.
    const allowed = await client.callTool({
      ...
      arguments: {
        ...
        edits: [
          {
            oldString: '@misc{a, title={Old Title Words Here}}\n',
            newString: '@misc{a, title={New Title Words Here}}\n',
          },
        ],
      },
    });
    ...
    const content = structured<{ content: string }>(read).content;
    expect(content).toBe('@misc{a, title={New Title Words Here}}\n');
    expect(content).not.toContain('%');
    expect(content).not.toContain('Old Title Words Here');
  });
