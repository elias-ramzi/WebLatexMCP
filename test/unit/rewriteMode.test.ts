...
describe('classifyEdit', () => {
  ...
  it('classifies a genuinely-rewritten pair of split display-math spans as minor', () => {
    // Regression coverage for the split-token display-math gap (see the `markupMask` test above):
    // "$$x = 1$$" tokenizes to ["$$x", "=", "1$$"], and before either fix none of its tokens were
    // masked, so an equation written this way could read as majority-prose and fall through to
    // the near-identical-overlap test — which a *genuine* rewrite (no shared vocabulary at all)
    // fails, wrongly landing on 'prose'. Kept alongside the single-equation test below (which the
    // toggler-collapse fix now also handles) as an independent construction: two short spans in
    // one 8-token edit reach the majority-markup threshold via `isMarkupToken`'s live-`$` rule
    // alone, even without the toggler-pairing bridging `=`.
    const oldString = '$$x = 1$$ and $$y = 2$$ text';
    const newString = '$$p = 9$$ but $$q = 8$$ note';
    expect(classifyEdit(oldString, newString)).toBe('minor');
  });

  it('classifies a genuinely-rewritten single long display equation as minor', () => {
    // The original gap fixture: "$$x = 1 + 2 + 3 + 4$$" tokenizes to 9 tokens, with only the two
    // boundary tokens ("$$x", "4$$") carrying a live "$" — not enough on their own to reach a
    // strict majority. Closing this fully needs `markupMask`'s toggle counter to collapse each
    // "$$" pair to one "$" before counting parity, so the two boundary tokens toggle (odd) and
    // pair up, masking every token between them — all 9 tokens end up markup, so `classifyEdit`
    // returns 'minor' via the majority-markup check alone, regardless of how different the
    // rewritten equation is.
    const oldString = '$$x = 1 + 2 + 3 + 4$$';
    const newString = '$$y = 5 + 6 + 7 + 8$$';
    expect(classifyEdit(oldString, newString)).toBe('minor');
  });
...
describe('markupMask', () => {
  ...
  it('masks a whole split display-math span, including a $-free token between its boundaries', () => {
    // Regression coverage for the reported gap: "$$x = 1$$" tokenizes to ["$$x", "=", "1$$"] —
    // "$$x" and "1$$" each have an EVEN count of unescaped "$" (2 apiece), so under a raw-parity
    // toggle counter neither is a "toggler" (an *attached* "$$" never toggled, only a standalone
    // "$$" token did) and neither is a complete span on its own (isCompleteMathToken requires the
    // *same* token to both start and end with "$"/"$$"). Before either fix, markupMask left every
    // one of these tokens unmasked.
    //
    // The full fix has two parts: `isMarkupToken` masks a token with any live "$" directly (which
    // catches "$$x" and "1$$" on their own merits), and `markupMask`'s toggle counter collapses
    // each "$$" pair to a single "$" before counting parity, so "$$x" and "1$$" now each toggle
    // once and pair up — masking "=" between them too, via the existing (unchanged) pairing loop.
    const tokens = ['$$x', '=', '1$$', 'and', 'more', 'prose', 'words', 'here'];
    expect(markupMask(tokens)).toEqual([true, true, true, false, false, false, false, false]);
  });

  it('does not treat a token with an escaped dollar and no live dollar as markup', () => {
    // "costs\$5" has a backslash mid-token (currency), not a leading control sequence, and after
    // stripping the escaped "\$" there is no live "$" left — this must NOT become markup under the
    // new "contains a live $" rule.
    const tokens = ['costs\\$5'];
    expect(markupMask(tokens)).toEqual([false]);
  });
});
