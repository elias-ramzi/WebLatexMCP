...
describe('FileService write + edit', () => {
  ...
  // Regression coverage for the "shiftPreserved assumes an overlapping splice began at or after
  // range.start" bug: that assumption is false when a `replaceAll` splice STRADDLES one of a
  // preserved range's boundaries rather than landing entirely before, entirely after, or fully
  // inside it. Two directions, both reproduced against the pre-fix code before this fix landed:
  //  - straddle-start: the splice starts before the range and ends inside it. The old code left
  //    `range.start` stale (it only ever grew `range.end`), so the range could end up with
  //    `start > end` and the intersects guard silently stopped firing — a later edit matching
  //    only the dead comment got wrongly applied.
  //  - straddle-end: the splice starts inside the range and ends after it. The old code still
  //    grew `range.end` by `delta`, inflating the range over live text it never touched — a
  //    later edit matching only that live text was wrongly refused.
  describe('opts.preserve (a replaceAll splice straddling a preserved range boundary)', () => {
    const PARA = 'Alpha beta gamma delta epsilon zeta eta theta.';

    it(
      'case (straddle-start): a replaceAll splice starting BEFORE a preserved range and ending ' +
        'INSIDE it must not silently let a later edit rewrite the dead remainder of the comment ' +
        '— it must throw the intersects-preserved refusal, and the file must be byte-identical ' +
        'to what it was before the call',
      async () => {
        const straddleStartPath = 'straddle-start.tex';
        const content = 'P'.repeat(100) + '\n' + PARA + '\ntail\n';
        await writeFile(path.join(dir, straddleStartPath), content);
        const preserve = createPreserveTransform('always');
        await expect(
          files.applyEdits(
            dir,
            straddleStartPath,
            [
              { oldString: PARA, newString: 'Totally different replacement words here.' },
              // Starts at the very beginning of the file (well before the preserved range, which
              // only starts after the 'P'.repeat(100) line) and ends inside the preserved
              // comment's own leading bytes ("% Alpha beta ") — straddles the range's *start*.
              {
                oldString: 'P'.repeat(100) + '\n% Alpha beta ',
                newString: 'x\n% ',
                replaceAll: true,
              },
              // Survives only inside what is left of the dead comment ("gamma" appears nowhere
              // in the live text) — must be refused, not silently applied to the comment.
              { oldString: 'gamma', newString: 'GAMMA' },
            ],
            { preserve },
          ),
        ).rejects.toThrow(
          /Edit 3: oldString only matches text preserved \(commented out\) by an earlier edit in this same call, not the live document; the live occurrence was already replaced by that edit\./,
        );
        expect(await readFile(path.join(dir, straddleStartPath), 'utf8')).toBe(content);
      },
    );

    it(
      'case (straddle-end): a replaceAll splice starting INSIDE a preserved range and ending ' +
        'AFTER it must not inflate the range over live text it never touched — a following edit ' +
        'that only matches that live text must still apply',
      async () => {
        const straddleEndPath = 'straddle-end.tex';
        const content = 'P'.repeat(100) + '\n' + PARA + '\ntail\n';
        await writeFile(path.join(dir, straddleEndPath), content);
        const preserve = createPreserveTransform('always');
        const res = await files.applyEdits(
          dir,
          straddleEndPath,
          [
            { oldString: PARA, newString: 'Totally different replacement words here.' },
            // Starts inside the preserved comment's trailing bytes ("theta.\n") and ends inside
            // the live replacement's first word ("Totally") — straddles the range's *end*.
            {
              oldString: 'theta.\nTotally',
              newString: 'theta.\n' + 'Z'.repeat(30) + 'LIVEMARK' + 'Z'.repeat(30) + 'Totally',
              replaceAll: true,
            },
            // Touches only the live marker text just spliced in — must be applied, not refused
            // as if it fell inside the (unrelated, untouched) preserved comment.
            { oldString: 'LIVEMARK', newString: 'Q' },
          ],
          { preserve },
        );
        expect(res.appliedEdits).toBe(3);
        expect(await readFile(path.join(dir, straddleEndPath), 'utf8')).toBe(
          'P'.repeat(100) +
            '\n% Alpha beta gamma delta epsilon zeta eta theta.\n' +
            'Z'.repeat(30) +
            'Q' +
            'Z'.repeat(30) +
            'Totally different replacement words here.\ntail\n',
        );
      },
    );
  });

  // Regression coverage for the non-unique-match message: "add more surrounding context ... or
  // set replaceAll" is fine advice for plain ambiguity, but when one of the ambiguous occurrences
  // sits inside a block an earlier edit in this same call already preserved (commented out),
  // "set replaceAll" is the one piece of advice that would silently rewrite that byte-exact block.
  describe('opts.preserve (the non-unique-match message names an occurrence inside a preserved block)', () => {
    it(
      'throws the "inside text preserved" variant, not the generic replaceAll suggestion, when ' +
        'one of two ambiguous occurrences sits inside a comment an earlier edit in this call just ' +
        'preserved, and leaves the file untouched',
      async () => {
        const messagePath = 'message.tex';
        const content =
          'intro\n' +
          'Alpha beta gamma delta epsilon zeta eta theta.\n' +
          'gamma is also mentioned again down here in the outro.\n';
        await writeFile(path.join(dir, messagePath), content);
        const preserve = createPreserveTransform('always');
        await expect(
          files.applyEdits(
            dir,
            ...
            [
              ...
              // "gamma" now matches twice: once live, in the untouched outro sentence, and once
              // inside the comment edit 1 just preserved.
              { oldString: 'gamma', newString: 'GAMMA' },
            ],
            { preserve },
          ),
        ).rejects.toThrow(
          /Edit 2: oldString matches 2 times in message\.tex, at least once inside text preserved \(commented out\) by an earlier edit in this same call; add more surrounding context so it matches only the live text, if any live occurrence remains\./,
        );
        expect(await readFile(path.join(dir, messagePath), 'utf8')).toBe(content);
      },
    );
  });
