...
describe('RewriteModeStore', () => {
  ...
  it('returns null when nothing is stored for a project, logging nothing', async () => {
  ...
  it('logs and resolves null when the stored path is a directory, not a file (EISDIR)', async () => {
    const file = rewriteModePath(workspace, 'p');
    ...
    expect(message).toEqual(expect.stringContaining(file));
    stderr.mockRestore();
  });
