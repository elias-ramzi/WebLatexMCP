...
import { TectonicCompiler } from '../../src/services/compiler.js';
import { parseLog } from '../../src/services/logParser.js';

// Gate the real compile on tectonic being installed, so this auto-skips in the fast
// CI job and on dev machines without it, but runs in the dedicated tex-smoke job.
...
describe.skipIf(!available)('tectonic compile smoke', () => {
  let dir: string;

  beforeAll(async () => {
    dir = await mkdtemp(path.join(os.tmpdir(), 'ovl-tectonic-'));
