...
`register_project`, which persists it (survives restarts, shared across sessions). The **token** still
comes from the `env` block or Git Credential Manager above — it is never stored with the project. See
