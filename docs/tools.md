...
| `register_project`  | Register a project and **persist** it to the workspace, so it survives a restart and is picked up by other sessions — no `WEB_LATEX_MCP_PROJECTS` needed. Pass `gitUrl` for a git project (cloned immediately unless `clone: false`) — the intended path for Claude Desktop: just paste your Overleaf git URL in the chat. Pass `path` instead for a **local** project: a directory already on this machine, compiled and edited in place with no clone (see [Local projects](#local-in-place-projects)). Exactly one of the two. See [Registering a project from the chat](#registering-a-project-from-the-chat).                                                                                                                                                                                                                                                                                                                                                                                               |
...
repo, notes in a scratch directory — is registered by **path** instead:

```jsonc
register_project({ "project": "cv", "path": "~/site/.context" })
```
...
- **The directory must already exist.** The server points at it; it never creates it.
