...
| `register_project`  | Register a project by `project` id + `gitUrl` and **persist** it to the workspace, so it survives a restart and is picked up by other sessions — no `WEB_LATEX_MCP_PROJECTS` needed. Clones immediately unless `clone: false`. The intended path for Claude Desktop: just paste your Overleaf git URL in the chat. See [Registering a project from the chat](#registering-a-project-from-the-chat).                                                                                                                                                                                                                                                                |
