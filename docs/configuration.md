...
| `WEB_LATEX_MCP_CONTACT_EMAIL`                              | no       | Contact address sent to Crossref and OpenAlex, which give identified clients a faster "polite pool". **Opt-in only** — nothing is sent unless you set this, and it is never derived from your git config. A value that is not a plausible, URL-safe address — or is longer than 254 characters, RFC 5321's limit — is ignored with a note on stderr. See [Reference backend](#reference-backend).                                                                                                                                                                                                                                                         |
