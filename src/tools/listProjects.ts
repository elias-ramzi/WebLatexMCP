  ...
  server.registerTool(
    'list_projects',
    {
      ...
      description:
        ...
        "registered, the result says how to add one. Each project's effective rewrite-preservation " +
        'mode (what edit_file does with replaced text) is reported as `rewriteMode`, with ' +
        '`rewriteModeSource` ("project" if stored for that project via set_rewrite_mode, ' +
        '"default" if nothing is stored) and `envConfigured` (whether WEB_LATEX_MCP_REWRITE_MODE ' +
        'set that default, as opposed to the built-in "off").',
      inputSchema: {},
      outputSchema,
    },
    async () => {
